...
- **Omnibar flicker.** The 250 ms state poll rebuilt the lock icon and restarted the load shimmer on every tick, and resize events while the page was in use re-placed the chrome webview even when its rectangle had not changed. The toolbar surface is opaque except while a menu is open, so page repaints no longer flash through the bar.

## 0.14.0 - 2026-09-19 — Ask AI, Chrome-parity lap on the Linux (WebKitGTK) lane
- **Ask AI (native, no proxy):** the `✦` button beside the address bar, `Alt+A`, the `@ai` omnibox scope, an "Ask <AI>" row at the bottom of every suggestion list, "Ask <AI> about this page" (app menu + page right-click) and "Ask <AI> about selection" (right-click) open the AI the user already subscribes to, in its own site, signed in as themselves. Providers (`src/engine/ai_search.rs`): Claude (`claude.ai/new?q=` prefill), ChatGPT (+ web-search variant), Perplexity, Grok, Google AI Mode (`udm=50`), Duck.ai, Mistral Le Chat, Kagi Assistant, and a custom `%s` template. Gemini and Copilot ignore `q=`, so a page script types the prompt into their composer and submits. Settings → Ask AI picks the provider and new-tab vs in-place.
- **Omnibox:** keyboard-navigable suggestions (↑/↓, Enter, Esc restores what you typed), inline autocomplete of history/bookmark hosts, "Switch to tab" rows for open tabs, `@tabs` / `@history` / `@bookmarks` scopes with a chip, "Search <engine> for …" default row, Alt+Enter opens in a new tab, Ctrl+Enter wraps `www.….com`, Shift+Delete removes a history suggestion. Clicking the lock (or Shift+Tab, Enter) opens a site-info popup: connection state, per-site camera/mic/location/notification permissions (Ask/Allow/Block), site zoom reset, shield, copy link.
- **Tabs:** audio indicator with click-to-mute (WebKit `set_is_muted`); right-click menu adds new tab to the right, reload/wake, mute, move to new window, put to sleep, close others, close to the right; hover tooltip shows the URL; Ctrl+PgUp/PgDn switch, Ctrl+Shift+PgUp/PgDn move. Memory saver discards background tabs after N minutes (Settings → Privacy; pinned, audible, private and loading tabs are exempt) and wakes them on switch.
- **Pages:** Chrome-style interstitials rendered in the Amni theme — "Can't reach this site" (`load-failed`), "Your connection is not private" (`load-failed-with-tls-errors`, with Back to safety / Proceed that pins the certificate for the host), "Aw, snap" on web-process termination. Permission prompts (geolocation, camera, microphone, notifications) as a chrome bubble, remembered per site in `permissions.json`; Web Notifications go to the desktop via `notify-send`. Link-target status bubble at the bottom-left; Ctrl+click / middle-click open links in a background tab (Ctrl+Shift+click foreground). Page right-click gains open in new/private tab, search selection, Ask AI, screenshot.
- **Find:** finds as you type, `3/17` match counter (red `0/0` when nothing matches), Ctrl+G / F3 next, Shift+G / Shift+F3 previous.
- **Downloads:** progress from WebKit `Download::received-data` (percent, bytes, indeterminate bar), cancel while running, a downloads toolbar button with an active badge, `Ctrl+S` saves the page as `.mhtml`, "Take screenshot" saves the visible page as PNG into Downloads, Settings → Downloads sets the folder and "ask where to save each file" (GTK save dialog).
- **More parity:** bookmarks bar (Ctrl+Shift+B / Settings → Look), per-site zoom memory, HTTPS-only mode (upgrade `http://` → `https://` with one-time fallback), hard reload (Ctrl+Shift+R), Ctrl+Shift+W close window, Ctrl+Shift+J devtools, Ctrl+Shift+Delete → Settings › Privacy, Ctrl+Shift+O → bookmarks, Ctrl+E/K and Alt+D focus the bar, Translate page (Google, in a new tab), Copy link, Recently closed list in the History panel (Ctrl+H) with titles, history page with search, day grouping and per-row delete.
- **Fixes:** every shortcut fired twice on GTK (tao's window handler and the in-page key script both reported it — Ctrl+T made two tabs, Ctrl+W closed two); the tile view re-rendered its grid on every 250 ms state poll, which flickered under the pointer (now keyed on tab changes; backdrop blur dropped); the chrome webview could show WebKit's native context menu; the tutorial described Servo on the WebKitGTK/Chromium lanes.
- **Security:** `amnibrowse://cmd|state|suggest|history|downloads|siteinfo` now require the per-run token on every platform. Before, any web page on Linux could issue browser commands (`fetch('amnibrowse://cmd/…')`) and read history.
...
- **Amni-OS (Linux) lane, same day:** `src/platform/chromium.rs` now builds and runs on Linux through wry on WebKitGTK (`cargo build --release` in the Amni-OS chroot, 9.4 MB binary). Windows-only COM pieces (`WebResourceRequested` shield, favicons, download progress, autofill, `ClearBrowsingData`, engine back/forward) are cfg-gated; Linux keeps the navigation-level ad block, DNT/GPC via wry, the native `amnibrowse://` scheme (Windows keeps the `http://amnibrowse.<host>/` mapping), decorated windows, JS-driven history/reload, and `WEBKIT_DISABLE_DMABUF_RENDERER=1` for VMs. A URL on the command line opens as a tab. Verified in the "Amni OS" VirtualBox VM: Amni chrome over WebKitGTK content, session restore, speedtest with logo/theme and blocked ad slots. `amni-os/packaging/amni-browse/PKGBUILD` switched to this lane (webkit2gtk-4.1, commit pin).
- **Linux request shield (2026-09-04):** `AdBlocker::content_rules()` turns the domain list, tracker paths and the auth allowlist into a WebKit content-blocker rule list (block + ignore-previous-rules + css-display-none for the iframes and ad containers the blocks leave behind). `compile_filter()` saves it once through `UserContentFilterStore` under the config dir and every tab's `UserContentManager` gets it; the shield toggle adds/removes it live (`reshield`). Unit test `content_rules_are_json` keeps the JSON valid. Dependency `webkit2gtk = "=2.0.1"` under `cfg(not(windows))`.
- **Known:** Chrome Web Store extensions need a custom WebView2 environment (Amni content scripts cover the injection use case); no tab search/vertical tabs.
