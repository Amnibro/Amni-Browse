...
- **B2 leftover (`<dialog>.showModal()`):** CLOSED on the native GTK omnibox (Wakka dogfood `/workspace/ab-b3-native`). Linux: `gtk`/`gdk`/`glib` 0.18; pack bar `amni-omni` (back/fwd/reload + Entry) at vbox index 0, **then** `build_gtk(vbox)`. Ctrl/Super+L: GTK window `key-press-event` → `grab_focus`+`select_region(0,-1)` (idle re-select so the next key replaces a stale suffix), return `glib::Propagation::Stop` (not `gtk::Inhibit`); tao `KeyboardInput` also `grab_focus` (no page JS). `__amni_native_omnibox` skips injected toolbar. No `showPopover` at init. NeverSSL cold start unchanged. Do not hide chrome; do not regress `build_gtk` paint.
