...
- **Known:** Chrome Web Store extensions need a custom WebView2 environment (Amni content scripts cover the injection use case); no tab search/vertical tabs; Linux lane has no request-level shield (WebKitGTK content filters are the next step) and blocked ad iframes leave white slots.
