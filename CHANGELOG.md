...
- **B2 leftover (`<dialog>.showModal()`):** page-JS `stealOmnibox` is dead against UA inert + focus trap. Linux packs a **native GTK omnibox** (back/forward/reload + Entry) at tao `default_vbox` index 0, **then** `build_gtk(vbox)` — outside WebKit so showModal cannot cover it. GTK window `key-press-event` Ctrl/Super+L `grab_focus`+`select_region` and **Inhibits** so WebKit never sees the key; tao `KeyboardInput` does the same (no `evaluate_script`). Entry activate → `Act::Nav` (http(s) as-is, dotted token → `https://`, else lite DDG search). JS toolbar skipped when `window.__amni_native_omnibox = true`. No `showPopover` at init. Do **not** hide chrome; do **not** regress `build_gtk` paint. NeverSSL cold start unchanged.
- **Non-Linux B2:** keep JS chrome; stronger `stealOmnibox` demotes open dialogs (`close`+`show`), clears `[inert]`, then focuses `#_au`.
