...
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** CLOSED on native GTK omnibox. `install_native_omnibox` packs `amni-omni` at vbox[0] (`pack_start` false) then `build_gtk(vbox)` — last-good sibling layout that paints. **No overlay. No native html margin-top.** Ctrl/Super+L `grab_focus` + `select_region(0,-1)`; skip JS `bindOmniboxHotkey` when `__amni_native_omnibox`. `Act::Omni` title/URL sync without `set_text` if entry focused; no lastWheel click-suppress. Non-Linux: JS chrome; steal demotes dialogs (`close`+`show`) and clears `[inert]`. No showPopover at init. NeverSSL unchanged.
