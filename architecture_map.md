...
- **Linux WEBVIEW (B3):** wry 0.46 `build(&window)` is X11-only and does not pack WebKit into tao's GTK vbox (title-only white window). Attach via `default_vbox()` + `build_gtk`. Cold start `WEBVIEW_COLD_START` = `http://neverssl.com/` (not lite DDG HTTPS). Ctrl/Cmd+L: capture-phase JS + native `KeyboardInput` focus `_au` in `#__atb_host` shadow (`__amni_l_bound`, toolbar stopPropagation, no showPopover).
