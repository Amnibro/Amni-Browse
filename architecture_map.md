...
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** page-JS steal is dead (inert + UA trap). Linux: native GTK omnibox at vbox[0] above WebKit (`build_gtk` after pack). Ctrl/Super+L inhibited on GTK window + tao grab_focus (no evaluate_script). `__amni_native_omnibox` skips injected toolbar. Non-Linux: JS chrome; steal demotes dialogs (`close`+`show`) and clears `[inert]`. No showPopover at init. NeverSSL cold start unchanged.
