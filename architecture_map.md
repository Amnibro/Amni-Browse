...
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** CLOSED on native GTK omnibox `amni-omni` at vbox[0] (`build_gtk` after pack). Ctrl/Super+L returns `glib::Propagation::Stop` (not `gtk::Inhibit`); tao `grab_focus` + `select_region(0,-1)`, no page JS. `__amni_native_omnibox` skips injected toolbar. `Act::Omni` from wry nav/load handlers updates title + GTK URL when the entry is not focused (no `load_url` from Omni). Capture-phase 120ms wheel→click/auxclick suppress before native return. Non-Linux: JS chrome; steal demotes dialogs (`close`+`show`) and clears `[inert]`. No showPopover at init. NeverSSL unchanged.
