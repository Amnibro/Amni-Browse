...
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** `stealOmnibox()` — popover=manual + `showPopover()` only at steal (not toolbar create). Ctrl/Cmd+L capture + pointerdown (`clientY < 48`, target not host). Native tao Ctrl+L calls `window.__amni_steal_focus`. Guardian-style CMP already passed; native dialog top-layer (18882) needs this.
