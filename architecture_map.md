...
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** CLOSED on native GTK omnibox `amni-omni` at vbox[0] (`build_gtk` after pack). Ctrl/Super+L returns `glib::Propagation::Stop` (not `gtk::Inhibit`); tao grab_focus, no page JS. `__amni_native_omnibox` skips injected toolbar. Non-Linux: JS chrome; steal demotes dialogs (`close`+`show`) and clears `[inert]`. No showPopover at init. NeverSSL unchanged.
