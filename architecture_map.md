...
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** CLOSED on native GTK omnibox `amni-omni` as `gtk::Overlay` (`valign` start) over `build_gtk(&overlay)` filling the vbox. Webview is the main child (sibling pack broke software-WebKit hit-test → clicks hit Back). Ctrl/Super+L `grab_focus` + `select_region(0,-1)`; skip JS `bindOmniboxHotkey` when `__amni_native_omnibox`. Native `html{margin-top:48px}`. `Act::Omni` title/URL sync without `set_text` if entry focused; no lastWheel click-suppress. Non-Linux: JS chrome; steal demotes dialogs (`close`+`show`) and clears `[inert]`. No showPopover at init. NeverSSL unchanged.
