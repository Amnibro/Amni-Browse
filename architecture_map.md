...
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** CLOSED on native GTK omnibox. Overlay in vbox → `build_gtk(&overlay)` first (WebKit main child) → then `add_overlay` `amni-omni` (`vexpand` false, `valign` Start, height 44) → `overlay.show_all()`. Showing/overlaying the bar first blanked WebKit. Ctrl/Super+L `grab_focus` + `select_region(0,-1)`; skip JS `bindOmniboxHotkey` when `__amni_native_omnibox`. Native `html{margin-top:48px}`. `Act::Omni` title/URL sync without `set_text` if entry focused; no lastWheel click-suppress. Non-Linux: JS chrome; steal demotes dialogs (`close`+`show`) and clears `[inert]`. No showPopover at init. NeverSSL unchanged.
