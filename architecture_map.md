...
- **Compositor (blank-viewport):** per frame `make_current` → chrome.paint → content.paint → `prepare_for_rendering()` (rebind window FB; v0.10.1 black-band fix) → blit offscreen at GL `Rect(0,0,w,content_h)` (bottom-left origin, NOT y=chrome_px) → present. Height 66/66 is intentional; stale docs mentioning 74 are historical.
- **Linux WEBVIEW (B3):** wry 0.46 `build(&window)` is X11-only and does not pack WebKit into tao's GTK vbox (title-only white window). Attach via `default_vbox()` + `build_gtk`. Cold start `WEBVIEW_COLD_START` = `http://neverssl.com/` (not lite DDG HTTPS).
- **WEBVIEW B2 leftover (`<dialog>.showModal()`):** CLOSED on native GTK omnibox. `install_native_omnibox` packs `amni-omni` at vbox[0] (`pack_start` false) then `build_gtk(vbox)` — last-good sibling layout that paints. **No overlay. No native html margin-top.** Ctrl/Super+L and focus-in `select_region(0,-1)`; skip JS `bindOmniboxHotkey` when `__amni_native_omnibox`. `Act::Omni` replaces (`delete_text` then `set_text`), skips empty/`about:blank`/`amnibrowse://`, no `set_text` if focused. `restore_session` default false. No lastWheel click-suppress. Non-Linux: JS chrome; steal demotes dialogs (`close`+`show`) and clears `[inert]`. No showPopover at init. NeverSSL unchanged.
- **Linux servo-real GL:** hardware ANGLE (`no-wgl` / mozangle) is Windows-only. Linux drops `no-wgl` and uses native GL via surfman/Mesa — software-GL present (llvmpipe verified), not ANGLE hardware.
