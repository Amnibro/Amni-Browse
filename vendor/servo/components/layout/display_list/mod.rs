...
impl<'a> BuilderForBoxFragment<'a> {
    ...
    fn build_background_for_painter(
        ...
        if let Some((image_key, layer)) = self.mask_image_layer(builder, state, painter.style) {
            if image_key == wr::ImageKey::DUMMY {
                return;
            }
    ...
    fn mask_image_layer(
        ...
        let ResolvedImage::Image { image, size } =
            ...
            // Gradient masks (border rings, fades) cannot be painted yet: painting the
            // unmasked box would be far worse than painting nothing.
            return Some((wr::ImageKey::DUMMY, background::BackgroundLayer { common: builder.common_properties(state, self.border_rect, style), bounds: LayoutRect::zero(), tile_size: LayoutSize::zero(), tile_spacing: LayoutSize::zero(), repeat: false, blend_mode: BackgroundBlendMode::Normal }));
        };
