...
pub(super) fn layout_layer(
    ...
    layout_layer_with(
        ...
        get_cyclic(&b.background_repeat.0, layer_index).clone(),
...
pub(super) fn representative_color(style: &ComputedValues) -> Option<AbsoluteColor> {
    ...
    for image in b.background_image.0.iter() {
        ...
        let stop = match &**gradient {
            GenericGradient::Linear { items, .. } => first_stop(items),
            GenericGradient::Radial { items, .. } => first_stop(items),
            GenericGradient::Conic { items, .. } => first_stop(items),
        };
...
fn first_stop<C, T>(items: &[GenericGradientItem<C, T>]) -> Option<&C> {
    items.iter().find_map(|item| match item {
        GenericGradientItem::SimpleColorStop(color) |
        GenericGradientItem::ComplexColorStop { color, .. } => Some(color),
...
fn layout_layer_with(
    ...
    let RepeatXY(repeat_x, repeat_y) = repeat_xy;
