param([int]$Port = 8920, [int]$Secs = 45)
$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $PSScriptRoot
$exe = Join-Path $root 'target\release\amni-browse.exe'
if (-not (Test-Path $exe)) { Write-Host 'GATE_E skip=no exe'; exit 1 }
$fix = Join-Path $root 'test\gateE_mixed'
$origin = "https://127.0.0.1:$Port"
$out = Join-Path $env:TEMP 'amni_gateE'
Remove-Item -Recurse -Force $out -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $out | Out-Null
$env:PYTHONUNBUFFERED = '1'
& python (Join-Path $fix 'make_fixture.py') 2>&1 | ForEach-Object { Write-Host $_ }
if ($LASTEXITCODE -ne 0) { Write-Host 'GATE_E skip=no cert'; exit 3 }
...
try {
  ...
  $probe = Invoke-WebRequest -Uri "$origin/favicon.ico" -MaximumRedirection 0 -UseBasicParsing -TimeoutSec 5 -ErrorAction SilentlyContinue
  Write-Host ('GATE_E fixture redirect=' + $probe.StatusCode)
  Start-Sleep -Milliseconds 400
  $cfg = Join-Path $env:APPDATA 'amni-browse'
  New-Item -ItemType Directory -Force -Path $cfg | Out-Null
  $sess = Join-Path $cfg 'session.json'
  if (Test-Path $sess) { Copy-Item $sess ($sess + '.gateEbak') -Force }
  ...
  $log = Join-Path $out 'gateE.log'
  $elog = Join-Path $out 'gateE.err.log'
  $env:RUST_LOG = 'info'
  ...
  $mine | ForEach-Object { Write-Host ('GATE_E log ' + $_) }
  $started = @($mine | Select-String -Pattern '\bstarted\b').Count
  $cached = @($mine | Select-String -Pattern 'embedder-(cache|fetch) ok').Count
  $mixed = @($mine | Select-String -Pattern 'mixed-redirect').Count
  $pending = @($fav | Select-String -Pattern '\bpending\b').Count
  $dom = @($fav | Select-String -Pattern 'appendChild').Count
  Write-Host ("GATE_E started=$started mixed-redirect=$mixed embedder-cache-ok=$cached pending=$pending appendChild=$dom")
  $ok = ($started -ge 1) -and ($mixed -ge 1) -and ($cached -eq 0) -and ($pending -eq 0) -and ($dom -eq 0)
  if ($ok) { Write-Host 'GATE_E PASS' } else { Write-Host 'GATE_E FAIL' }
  if (Test-Path ($sess + '.gateEbak')) { Copy-Item ($sess + '.gateEbak') $sess -Force }
