param([int]$Port = 8917, [int]$Secs = 45)
$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $PSScriptRoot
$exe = Join-Path $root 'target\release\amni-browse.exe'
if (-not (Test-Path $exe)) { Write-Host 'GATE_B skip=no exe'; exit 1 }
$fix = Join-Path $root 'test\gateB_nolic'
$origin = "http://127.0.0.1:$Port"
$out = Join-Path $env:TEMP 'amni_gateB'
...
try {
  ...
  Write-Host ('GATE_B fixture bytes=' + $probe.RawContentLength)
  Start-Sleep -Milliseconds 400
  $hitsAfterProbe = Count-FaviconGets @($srvLog, $srvErr)
  Write-Host ('GATE_B serverFaviconGET after probe=' + $hitsAfterProbe)
  $cfg = Join-Path $env:APPDATA 'amni-browse'
  New-Item -ItemType Directory -Force -Path $cfg | Out-Null
  $sess = Join-Path $cfg 'session.json'
  if (Test-Path $sess) { Copy-Item $sess ($sess + '.gateBbak') -Force }
  ...
  $log = Join-Path $out 'gateB.log'
  $elog = Join-Path $out 'gateB.err.log'
  $env:RUST_LOG = 'info'
  ...
  $mine | ForEach-Object { Write-Host ('GATE_B log ' + $_) }
  ...
  Write-Host ("GATE_B started=$started embedder-cache-ok=$cached bad=$bad pending=$pending appendChild=$dom serverFaviconGET=$hits serverFaviconGET_total=$hitsTotal embedder-net=$embnet")
  $ok = ($started -ge 1) -and ($cached -ge 1) -and ($pending -eq 0) -and ($dom -eq 0) -and ($hits -ge 1) -and ($embnet -ge 2)
  if ($ok) { Write-Host 'GATE_B PASS' } else { Write-Host 'GATE_B FAIL' }
  if (Test-Path ($sess + '.gateBbak')) { Copy-Item ($sess + '.gateBbak') $sess -Force }
