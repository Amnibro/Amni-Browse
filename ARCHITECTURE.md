...
- **WEBVIEW B2 leftover:** page-JS steal cannot beat `<dialog>.showModal()` (inert + UA focus trap). Linux: native GTK omnibox packed at vbox index 0 **above** WebKit, then `build_gtk(vbox)`. Ctrl/Super+L is handled on the GTK window (`Inhibit`) and tao (`grab_focus`, no evaluate_script). Init script sets `__amni_native_omnibox` and skips the injected toolbar. Non-Linux: JS chrome; steal demotes `dialog[open]` via `close`+`show`, clears `[inert]`, focuses `#_au`.
