...
- **WEBVIEW B2 leftover:** CLOSED on native GTK omnibox `amni-omni` at vbox index 0, then `build_gtk(vbox)`. Ctrl/Super+L returns `glib::Propagation::Stop` (not `gtk::Inhibit`); tao `grab_focus`, no page JS. `gtk`/`gdk`/`glib` 0.18. `__amni_native_omnibox` skips injected toolbar. Non-Linux: JS chrome; steal demotes `dialog[open]` via `close`+`show`, clears `[inert]`, focuses `#_au`.
