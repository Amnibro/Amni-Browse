...
- **WEBVIEW B2 leftover:** CLOSED on native GTK omnibox. `install_native_omnibox` packs `amni-omni` into `default_vbox()` at index 0 (`pack_start` expand false), **then** `build_gtk(vbox)`. Same sibling layout as the last-good native binary that paints. **No** `gtk::Overlay`. **No** native `html{margin-top}`. Ctrl/Super+L and focus-in `select_region(0,-1)`. Skip JS `bindOmniboxHotkey` when `__amni_native_omnibox`. `Act::Omni` from nav + page-load: skip empty/`about:blank`/`amnibrowse://`; replace via `delete_text(0,-1)` then `set_text` only when the entry does not have focus — never `load_url`. `restore_session` defaults **false**. No lastWheel click-suppress. Non-Linux: JS chrome; steal demotes `dialog[open]` via `close`+`show`, clears `[inert]`, focuses `#_au`.
