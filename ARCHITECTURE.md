...
- **WEBVIEW B2 leftover:** CLOSED on native GTK omnibox. Pack `gtk::Overlay` in the vbox, `build_gtk(&overlay)` **first** (WebKit is the main child), **then** `add_overlay` `amni-omni` (`vexpand` false, `valign` Start, height 44) and `overlay.show_all()`. Do not `show_all` / `add_overlay` before the webview exists (that blanked content). Ctrl/Super+L `grab_focus` + `select_region(0,-1)`. Skip JS `bindOmniboxHotkey` when `__amni_native_omnibox`. Native `html{margin-top:48px}`. `Act::Omni` set_title + GTK `set_text` only when the entry does not have focus — never `load_url`. No lastWheel click-suppress. Non-Linux: JS chrome; steal demotes `dialog[open]` via `close`+`show`, clears `[inert]`, focuses `#_au`.
