...
- **WEBVIEW B2 leftover:** CLOSED on native GTK omnibox `amni-omni` at vbox index 0, then `build_gtk(vbox)`. Ctrl/Super+L returns `glib::Propagation::Stop` (not `gtk::Inhibit`); tao `grab_focus` + `select_region(0,-1)` (idle re-select), no page JS. `gtk`/`gdk`/`glib` 0.18. `__amni_native_omnibox` skips injected toolbar. In-page nav: wry `with_navigation_handler` + `with_on_page_load_handler` push `Act::Omni` (set_title; GTK `set_text` only when the entry does not have focus — never `load_url`). Wheel-over-link: capture-phase suppress of click/middle-auxclick within 120ms of `wheel` (bound before native-omnibox return). Non-Linux: JS chrome; steal demotes `dialog[open]` via `close`+`show`, clears `[inert]`, focuses `#_au`.
