...
- **WEBVIEW B2 leftover:** CLOSED on native GTK omnibox `amni-omni` as a `gtk::Overlay` child (`valign` start) over wry `build_gtk(&overlay)` filling tao's vbox. Webview is the overlay **main** child so paint and hit-test match (a vbox-sibling bar made software WebKit land page clicks on Back). Ctrl/Super+L returns `glib::Propagation::Stop`; tao `grab_focus` + `select_region(0,-1)` (idle re-select). `bindOmniboxHotkey` is skipped when `__amni_native_omnibox` (B2 steal is the GTK bar, not the old 48px page steal). Native injects `html{margin-top:48px}` so content is not under the overlay. `Act::Omni` set_title + GTK `set_text` only when the entry does not have focus — never `load_url`. No lastWheel click-suppress. Non-Linux: JS chrome; steal demotes `dialog[open]` via `close`+`show`, clears `[inert]`, focuses `#_au`.
