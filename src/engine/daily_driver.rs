...
pub fn fetch_pdf_bytes(url: &str) -> Result<Vec<u8>, String> {
    let client = reqwest::blocking::Client::builder().timeout(std::time::Duration::from_secs(20)).build().map_err(|e| e.to_string())?;
    let resp = client.get(url).send().map_err(|e| e.to_string())?;
    if !resp.status().is_success() { return Err(format!("pdf http {}", resp.status())); }
    let bytes = resp.bytes().map_err(|e| e.to_string())?;
    if bytes.len() > 20 * 1024 * 1024 { return Err("pdf larger than 20MB".into()); }
    Ok(bytes.to_vec())
}
pub fn pdf_viewer_html(url: &str, theme_vars: &str, tok: &str) -> String {
    match fetch_pdf_bytes(url) {
        Ok(bytes) => pdf_js_html(url, &bytes, theme_vars, tok),
        Err(_) => pdf_fallback_html(url, theme_vars, tok),
    }
}
fn pdf_fallback_html(url: &str, theme_vars: &str, tok: &str) -> String {
    let safe = url.replace('&', "&amp;").replace('<', "&lt;").replace('>', "&gt;").replace('"', "&quot;");
    format!("<!DOCTYPE html><html><head><meta charset='utf-8'><title>PDF</title><style>:root{{{theme}}}body{{font:15px/1.5 -apple-system,'Segoe UI',sans-serif;background:var(--bg,#08090B);color:var(--text,#EDEFF2);margin:0;padding:48px 24px;text-align:center}}h1{{font-size:22px;color:var(--accent,#C89B4E);margin:0 0 8px}}p{{color:var(--dim,#A7ADB6);max-width:520px;margin:0 auto 22px;word-break:break-all}}button{{display:inline-block;margin:0 6px 8px;padding:10px 18px;border-radius:8px;border:1px solid var(--stroke,#20242B);background:var(--elev,#111418);color:var(--text,#EDEFF2);cursor:pointer;font:inherit}}button.primary{{background:var(--accent,#C89B4E);color:#08090B;border-color:transparent;font-weight:600}}</style></head><body><h1>PDF</h1><p>{safe}</p><p>Could not fetch bytes for the in-engine viewer. Open with the system reader or save again.</p><button class='primary' onclick=\"fetch('amnibrowse://cmd/open_download?tok={tok}&url='+encodeURIComponent('{js}'),{{mode:'no-cors'}})\">Open with system</button><button onclick=\"fetch('amnibrowse://cmd/download?tok={tok}&url='+encodeURIComponent('{js}'),{{mode:'no-cors'}})\">Save again</button></body></html>", theme = theme_vars, safe = safe, tok = tok, js = url.replace('\\', "\\\\").replace('\'', "\\'"))
}
fn pdf_js_html(url: &str, bytes: &[u8], theme_vars: &str, tok: &str) -> String {
    let b64 = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, bytes);
    let safe = url.replace('&', "&amp;").replace('<', "&lt;").replace('>', "&gt;").replace('"', "&quot;");
    format!("<!DOCTYPE html><html><head><meta charset='utf-8'><title>PDF</title><script src='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js'></script><style>:root{{{theme}}}body{{margin:0;background:var(--bg,#08090B);color:var(--text,#EDEFF2);font:13px/1.4 -apple-system,'Segoe UI',sans-serif}}#bar{{display:flex;gap:8px;align-items:center;padding:8px 12px;border-bottom:1px solid var(--stroke,#20242B);background:var(--elev,#111418)}}#bar span{{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dim,#A7ADB6)}}button{{background:var(--bg,#08090B);color:var(--text,#EDEFF2);border:1px solid var(--stroke,#20242B);border-radius:6px;padding:6px 10px;cursor:pointer}}#pages{{padding:12px;display:flex;flex-direction:column;align-items:center;gap:12px}}canvas{{max-width:100%;box-shadow:0 4px 24px #0008}}</style></head><body><div id='bar'><button id='prev'>Prev</button><button id='next'>Next</button><span id='meta'>PDF</span><button onclick=\"fetch('amnibrowse://cmd/open_download?tok={tok}&url='+encodeURIComponent('{js}'),{{mode:'no-cors'}})\">System</button></div><div id='pages'></div><script>pdfjsLib.GlobalWorkerOptions.workerSrc='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';var raw=atob('{b64}');var u8=new Uint8Array(raw.length);for(var i=0;i<raw.length;i++)u8[i]=raw.charCodeAt(i);var pageN=1,doc=null;function paint(){{if(!doc)return;doc.getPage(pageN).then(function(p){{var vp=p.getViewport({{scale:1.35}});var c=document.createElement('canvas');c.width=vp.width;c.height=vp.height;var box=document.getElementById('pages');box.innerHTML='';box.appendChild(c);p.render({{canvasContext:c.getContext('2d'),viewport:vp}});document.getElementById('meta').textContent='{safe} · '+pageN+' / '+doc.numPages;}});}}pdfjsLib.getDocument({{data:u8}}).promise.then(function(d){{doc=d;paint();}});document.getElementById('prev').onclick=function(){{if(doc&&pageN>1){{pageN--;paint();}}}};document.getElementById('next').onclick=function(){{if(doc&&pageN<doc.numPages){{pageN++;paint();}}}};</script></body></html>", theme = theme_vars, tok = tok, js = url.replace('\\', "\\\\").replace('\'', "\\'"), b64 = b64, safe = safe.replace('\\', "\\\\").replace('\'', "\\'"))
}
