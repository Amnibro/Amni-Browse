...
impl AppState {
    ...
    fn tutorial_html(&self) -> String {
        ...
        TUTORIAL_TPL.replace("__THEME__", &self.theme_root_vars()).replace("__VER__", env!("CARGO_PKG_VERSION")).replace("__TOK__", &self.cmd_token).replace("__BROWSERS__", &browsers).replace("__ENGINE__", "Real Servo").replace("__ENGINEBLURB__", "This is a real browser engine &#8212; Servo &#8212; not a Chromium wrapper.").replace("__MEDIANOTE__", "YouTube plays in Servo when we can extract a progressive stream. Netflix-class DRM stays in the same window, same chrome, as an in-tab pane.").replace("__STEP3__", "Settings &#8594; Password manager: Amni vault, Bitwarden (<code>bw</code>), 1Password (<code>op</code>), or KeePassXC. Unlock once. A key icon appears in the URL bar when a page has matches &#8212; pick one to fill, like Chrome.")
    }
